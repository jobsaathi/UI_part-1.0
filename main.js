...
app.post("/signup",function(req,res){
	var user=req.body.usernameup
	var pass=req.body.newpass
ibmdb.open(conectionstring, function (err,conn) {
  if (err) return console.log(err);
   conn.query("select * from TGP46415.user_login where username='"+user+"'", function (err, rows) {
	   if(err){
		    console.log("user not exists")
  conn.query("insert into TGP46415.user_login values("+mastid+",'"+user+"','"+pass+"')", function (err, rows) {
    if (err) console.log(err);
    else {
		//console.log(data.ID);
		//console.logJSON.stringify(q1)(rows);
...
ibmdb.open(conectionstring, function (err,conn) {
  ...
  conn.query("select cand_id,password from TGP46415.user_login where username='"+user+"' ", function (err, rows) {
    if (err) {console.log(err);
	res.status(400).send({
...
app.post("/candidateprofsearch",function(req,res){
	
	var cmasterid=req.body.masterid;
	console.log(cmasterid)
ibmdb.open(conectionstring, function (err,conn) {
  if (err) return console.log(err);
  var fetchstr="select a.cand_first_nam,a.cand_midd_nam,a.cand_last_nam,a.cand_birth_dt,a.cand_prm_mob_num,a.cand_prm_email_id,b.cand_addr_line1,b.cand_city_nam,b.cand_dist_nam,b.cand_stat_nam,b.cand_post_cd,c.cand_prim_skill_desc,c.cand_alt_skill_desc,c.cand_skill_industry,c.cand_prof_job_loc from TGP46415.cand_prof_master a ,  TGP46415.cand_addr_det b ,TGP46415.cand_edu_skill_det  c where a.cand_mast_id=b.cand_mast_id and b.cand_mast_id=c.cand_master_id and a.cand_mast_id="+cmasterid;
  console.log(fetchstr);
   conn.query(fetchstr, function (err, rows) {
	   if(err){
		    console.log(err)
  res.status(400).send({
   message: 'User already exists'
...
app.post("/companyprofsearch",function(req,res){
	
	var pskill= req.body.primskill
	var sskill= req.body.secskill
	var industry= req.body.industry
	var joblocation= req.body.joblocation
	console.log(pskill)
ibmdb.open(conectionstring, function (err,conn) {
  if (err) return console.log(err);
  var fetchstr="select  comp_master_id from TGP46415.comp_job_master where comp_job_industry='"+industry+"' and comp_job_prim_skill='"+pskill+"' and comp_job_alt_skill='"+sskill+"' and comp_job_location='"+joblocation+"'";
  console.log(fetchstr);
   conn.query(fetchstr, function (err, rows) {
	   if(err){
		    console.log(err)
